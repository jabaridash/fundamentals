    ...
    override func spec() {
        describe("An ExampleService") {
            ...
            afterEach {
                ...
                UserDefaults.standard.removePersistentDomain(forName: "example")
            }
            ...
            it("integrates the components from the Fundamentals library as expected") {
                ...
                waitUntil { done in
                    subject.load { result in
                        ...
                        let cashedUser = try! JSONDecoder().decode(
                            User.self,
                            from: UserDefaults.standard.object(forKey: "user") as! Data
                        )
                        
                        expect(subject.usersLoaded) == 1
                        expect(actualUser) == expectedUser
                        expect(cashedUser) == expectedUser
                        done()
                    }
                }
            }
